...
@dataclass
class RigolLLM2Config(BaseModel):
    api_key: str = ""
    base_url: str = "https://api.openai.com/v1"
    extra_headers: str = "{}"  # New field for custom headers (JSON string)
    model: str = (
...
class RigolChatGPT:
    ...
    def __init__(self, ten_env: AsyncTenEnv, config: RigolLLM2Config):
        self.config = config
        self.ten_env = ten_env
        ten_env.log_info(
            f"RigolChatGPT initialized with config: {config.api_key}"
        )
        ...
        if hasattr(config, 'extra_headers') and config.extra_headers:
            try:
                extra = json.loads(config.extra_headers)
                headers.update(extra)
                ten_env.log_info(f"Injected extra headers: {list(extra.keys())}")
            except Exception as e:
                ten_env.log_error(f"Failed to parse extra_headers: {e}")
