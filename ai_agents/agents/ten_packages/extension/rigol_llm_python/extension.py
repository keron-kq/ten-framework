...
from .rigol import RigolChatGPT, RigolLLM2Config


class RigolLLM2Extension(AsyncLLM2BaseExtension):
    ...
    async def on_start(self, async_ten_env: AsyncTenEnv) -> None:
        ...
        self.config = RigolLLM2Config.model_validate_json(config_json)
        ...
        # Create instance
        try:
            self.client = RigolChatGPT(async_ten_env, self.config)
            async_ten_env.log_info(
                f"initialized with max_tokens: {self.config.max_tokens}, model: {self.config.model}"
            )
        except Exception as err:
            async_ten_env.log_info(f"Failed to initialize RigolChatGPT: {err}")
