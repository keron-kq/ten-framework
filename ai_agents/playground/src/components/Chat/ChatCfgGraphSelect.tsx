...
import { GRAPH_DISPLAY_NAME_MAP } from "@/data/greetingScripts";
...
export function RemoteGraphSelect() {
  ...
  const graphOptions = graphs.map((item) => ({
    label: GRAPH_DISPLAY_NAME_MAP[item.name] || item.name,
    value: item.graph_id,
  }));

  // Get the selected graph's label for display
  const selectedGraph = graphs.find((g) => g.graph_id === graphName);
  const rawName = selectedGraph?.name || "Select Graph";
  const displayLabel = GRAPH_DISPLAY_NAME_MAP[rawName] || rawName;
  ...
  return (
    <>
      ...
      >
        <SelectTrigger
          className={cn(
            "w-auto min-w-[240px] max-w-[400px] md:min-w-[300px]"
          )}
        >
