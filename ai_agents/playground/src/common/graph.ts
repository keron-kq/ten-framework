import type { ModuleRegistry } from "./moduleConfig";
...
class GraphEditor {
  ...
  static updateNodeProperty(
    ...
    const node = GraphEditor.findNode(graph, nodeName);
  ...
  static addOrUpdateConnection(
    ...
    const connection = GraphEditor.findConnection(graph, source);
  ...
  static removeNodeAndConnections(graph: Graph, addon: string): void {
    // Remove the node
    const node = GraphEditor.removeNode(graph, addon);
  ...
  static linkTool(
    ...
    llmNode: Node,
    toolNode: Node,
    tool: ModuleRegistry.ToolModule
  ): void {
